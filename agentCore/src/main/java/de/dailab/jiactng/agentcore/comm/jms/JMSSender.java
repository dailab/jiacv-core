...
//    private Destination _defaultReplyTo = null;
    ...
    private void doInit() throws JMSException {
        _log.debug("initialise JMSSender");
...
//    }
    ...
    public void sendMessage(IJiacMessage message, Destination destination, long timeToLive) throws JMSException {
        _log.debug("sending message to " + destination);
...
//        log.debug("using replytoDestination " + replyToDestination);
        ...
        _log.debug("begin of sending procedure... now!");
        MessageProducer producer = null;
        try {
...
//            objectMessage.setJMSReplyTo(replyToDestination);
            ...
            try {
                // TODO figure out whether we can use a blank (with no default destination) MessageProvider without closing it
                producer.close();
            } catch (JMSException jmse) {
                _log.warn("exception occured while closing producer", jmse);
            }
        }
        _log.debug("sending procedure done");
    }
}
