...
public class NodeProtocol implements INodeProtocol {
	...
	/*
	 * tempor�re hilfsmethode. wenn sp�ter ordentlich auf commands reagiert wird, sollt edort festgelegt werden, mit
	...
	private SendInfo doAcknowledge(IJiacMessage receivedMsg) {
		...
		if (ACK_GET_SERVICES.equals(operation)) {
			...
			log.debug("Ich werd' verr�ckt.. hab ein NOP-Ack gekriegt, von " + receivedMsg.getStartPoint());
		}
		return sendInfo;
	}
	 ...
	 * @return eigentlich soll die zu verschickende Antwort zur�ckgegeben werden, momentan wird direkt aus dieser methode
	 ...
	 * 
	 * @param receivedMsg in der receivedMsg sind empf�ngeradresse drin
	 ...
	 * @return PROCESSING_SUCCESS, wenn versendet; PROCESSING_FAILED wenn Versenden nicht m�glich war.
	 */
