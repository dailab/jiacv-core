...
public class PingPongTestCase extends TestCase {
    ...
    protected void setUp() throws Exception {
        ...
        IGroupAddress address= CommunicationAddressFactory.getDefault().createGroupAddress(DUMMY_ADDRESS);
        for(IAgent agent : _node.findAgents()) {
            for(IAgentBean bean : agent.getAgentBeans()) {
                ...
                } else if(bean instanceof AbstractCommunicationBean) {
                    ((AbstractCommunicationBean) bean).joinGroup(address);
                }
            }
        }
