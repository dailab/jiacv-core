...
/**
 * This class supports the periodically management of the JMX connector servers. This includes getting the actual list of active and multicast supporting network interfaces,
 * creating and removing connector server, and sending multicast packets of the JMX URLs via the corresponding network interface.
 * 
 * @author Jan Keiser
 */
public final class JmxConnectorManager extends TimerTask {
   ...
   /**
    * Constructor for JMX connector manager. It creates a multicast socket for a given port and a given time to live for sending JMX connector URLs.
    * 
   ...
   /**
    * For each network interface, it joins the multicast group, updates the connector server and sends multicast packets via the network interface.
    */
   @Override
   public void run() {
   ...
   private JMXConnectorServer createConnector(JmxConnector conf, InetAddress address, String ifcName) {
      ...
      if (protocol.equals("rmi")) {
         ...
         if ((registryPort > 0) || (registryHost != null)) {
            path = "/jndi/rmi://" + ((registryHost == null) ? address.getHostAddress() : registryHost) + ((registryPort > 0) ? ":" + registryPort : "") + "/" + node.getUUID() + "/" + ifcName;
         }
      }
