...
You can launch the AgentNodes either with Eclipse (run configurations can be found in "bin/docs/eclipse_RunConfigurations/..") or with the start scripts that are generated by Maven (you will find them in "target/pingPong_WithTLS-*-bin/pingPong_WithTLS-*/bin").
The AgentNode Configuration explicitly defines the paths to the stores. All you have to do is to compile the project and to execute both AgentNodes (optionally on separate computers or on the same).
