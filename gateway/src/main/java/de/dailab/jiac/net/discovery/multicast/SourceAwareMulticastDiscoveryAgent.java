...
public class SourceAwareMulticastDiscoveryAgent implements DiscoveryAgent, Runnable {
    ...
    class RemoteBrokerData {
        ...
        public RemoteBrokerData(String brokerName, String service, InetAddress sourceAddress) {
    ...
    private void processAlive(String brokerName, String service, InetAddress source) {
        if (selfService == null || !service.equals(selfService)) {
            ...
            if (data == null) {
                data = new RemoteBrokerData(brokerName, service, source);
    ...
    private void processDead(String service, InetAddress source) {
    ...
    private void doExpireOldServices() {
        ...
        for (Iterator<RemoteBrokerData> i = brokersByService.values().iterator(); i.hasNext();) {
            ...
            if (data.getLastHeartBeat() < expireTime) {
                processDead(data.service, data.sourceAddress);
            }
        }
    }
    ...
    private void fireServiceRemovedEvent(RemoteBrokerData data) {
        if (discoveryListener != null) {
            ...
            getExecutor().execute(new Runnable() {
                public void run() {
                    DiscoveryListener discoveryListener = SourceAwareMulticastDiscoveryAgent.this.discoveryListener;
                    if (discoveryListener != null) {
